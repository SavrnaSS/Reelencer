...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!reelUrl || !screenshotUrl) {
    return NextResponse.json({ error: "reelUrl and screenshotUrl are required" }, { status: 400 });
  }
  ...
  const nextGates = { ...(item.gates ?? {}), proofAttached: true };
  const submission = { reelUrl, screenshotUrl, submittedAt: nowStamp };
  const audit = [{ at: nowStamp, by: "Worker", text: "Submitted proof links." }, ...(item.audit ?? [])];
